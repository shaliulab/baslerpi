...
def drive_basler(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        except (genicam._genicam.AccessException, genicam._genicam.LogicalErrorException) as error:
            match_objects = [
                re.match(".*(ExposureTime).*", error.args[0]),
                re.match(".*(FrameRate).*", error.args[0]),
            ]
            attribute = [e.groups()[0].lower() for e in match_objects if e is not None][0]
