...
class OpenCVCompressor:
    ...
    def _find(self, frame):
        ...
        frame = cv2.erode(frame, self._erode_kernel, iterations=self._erode_iterations)

        segmented = self._backSub.apply(frame)
        mask = np.bitwise_and(segmented, np.copy(self._frame_mask))

        mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, self._closing_kernel, iterations=self._closing_iterations)
        mask = cv2.erode(mask, self._erode_kernel, iterations=3)
        cts, _ = cv2.findContours(mask, mode=cv2.RETR_EXTERNAL, method=cv2.CHAIN_APPROX_SIMPLE)
        cts = [ct for ct in cts if cv2.contourArea(ct) > self._minContour_area]

        moms = [cv2.moments(ct) for ct in cts]
        positions = [(int(M['m10']/M['m00']), int(M['m01']/M['m00'])) for M in moms]
        ...
        if self._debug:
            ...
            self._debug1 = np.hstack([
                mask,
                cv2.drawContours(np.zeros(frame.shape, dtype=np.uint8), cts, -1, 255, -1)
            ])
        return positions
    ...
    def apply(self, frame):
        ...
        for j, rect in enumerate(rectangles):
            final_mask = cv2.rectangle(final_mask, (rect[0], rect[1]), (rect[0]+rect[2], rect[1]+rect[3]), 255, -1)
            final_mask = cv2.putText(final_mask, str(j), (rect[0], rect[1]), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, 128)

        compressed_frame = np.bitwise_and(frame_orig, final_mask)

        if self._debug:

            if ((self._framecount % self._frequency) == 0 or self.warmup) and self._debug:
                compressed_frame = cv2.putText(compressed_frame, str("FIND"), (100, 200), cv2.FONT_HERSHEY_COMPLEX_SMALL, 5, 255)
                compressed_frame = cv2.putText(compressed_frame, str(len(positions)), (1000, 200), cv2.FONT_HERSHEY_COMPLEX_SMALL, 5, 255)
                compressed_frame = cv2.putText(compressed_frame, str(len(self._last_positions)), (1200, 200), cv2.FONT_HERSHEY_COMPLEX_SMALL, 5, 255)
                for pt in self._last_positions:
                    compressed_frame = cv2.circle(compressed_frame, pt, 20, 128, -1)
            ...
            debugging_image = cv2.resize(debugging_image, tuple((np.array(debugging_image.shape)[::-1] // 5).tolist()) , interpolation = cv2.INTER_AREA)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--timeout", type=int, default=30000, help="Camera tries getting a frame for ms")
    ap.add_argument("--duration", type=int, default=300000, help="Camera fetches frames for this ms")
    ...
    compressor = OpenCVCompressor(ntargets=3, frequency=frequency, shape=frame.shape, debug=args.debug)
