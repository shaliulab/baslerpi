...
class TimeAnnotator(BaseAnnotator):
    ...
    def annotate(self, frame):
        ...
        frame = cv2.putText(frame, time_now, (500, 150), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 1, cv2.LINE_AA)
...
class DateAnnotator(BaseAnnotator):
    ...
    def annotate(self, frame):
        ...
        frame = cv2.putText(frame, date_now, (0, 150), cv2.FONT_HERSHEY_SIMPLEX, 2, 255, 1, cv2.LINE_AA)
        frame = super().annotate(frame)
        return frame
...
class FreeAnnotator(BaseAnnotator):
    ...
    def annotate(self, frame):
        frame = cv2.putText(frame, self._text, (1000, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, 255, 2, cv2.LINE_AA)
        frame = super().annotate(frame)
        return frame
...
class Annotator:
    ...
    def _decompose_value(self, value):
        ...
        if value_is_str:
            ...
            keys = [2**i for i, v in enumerate(binary_string[::-1]) if v == "1"]
